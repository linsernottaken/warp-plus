...
func APIRunWarp(ctx context.Context, l *slog.Logger, opts APIWarpOptions) error {
	if opts.WireguardConfig != "" {
		if err := APIrunWireguard(ctx, l, opts); err != nil {
			return err
		}
	...
	if opts.Scan != nil {
		...
		res, err := wiresocks.APIRunScan(ctx, l, *opts.Scan)
		if err != nil {
			return err
		}
	...
	case opts.Psiphon != nil:
		...
		warpErr = APIrunWarpWithPsiphon(ctx, l, opts, endpoints[0])
	case opts.Gool:
		l.Info("running in warp-in-warp (gool) mode")
		// run warp in warp
		warpErr = APIrunWarpInWarp(ctx, l, opts, endpoints)
	default:
		l.Info("running in normal warp mode")
		// just run primary warp on bindAddress
		warpErr = APIrunWarp(ctx, l, opts, endpoints[0])
	}
...
func APIrunWireguard(ctx context.Context, l *slog.Logger, opts APIWarpOptions) error {
	...
	_, err = wiresocks.APIStartProxy(ctx, l, tnet, opts.Bind)
	if err != nil {
		return err
	}
...
func APIrunWarpInWarp(ctx context.Context, l *slog.Logger, opts APIWarpOptions, endpoints []string) error {
	...
	_, err = wiresocks.APIStartProxy(ctx, l, tnet2, opts.Bind)
	if err != nil {
		return err
	}
...
func APIrunWarpWithPsiphon(ctx context.Context, l *slog.Logger, opts APIWarpOptions, endpoint string) error {
	...
	// Run a proxy on the userspace stack
	warpBind, err := wiresocks.APIStartProxy(ctx, l, tnet, netip.MustParseAddrPort("127.0.0.1:0"))
	if err != nil {
		return err
	}
